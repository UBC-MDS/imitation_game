...
def encrypt_asymmetric(
    message: str,
    receiver_public_key: Union[bytes, str],
    sender_private_key: Union[bytes, str],
) -> str:
    """
    Encrypts a message using the receiver's public key and signs it with the
    sender's private key. This ensures both confidentiality (only receiver can
    decrypt) and authenticity (receiver can verify sender).
    ...
    receiver_public_key : bytes or str
        The receiver's RSA public key in PEM format (bytes) or path to key file
        (str).
    sender_private_key : bytes or str
        The sender's RSA private key in PEM format (bytes) or path to key file
        (str) for signing.
